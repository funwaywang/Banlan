...
namespace Banlan
{
    public class BitmapData
    {
        ...
        public BitmapData(SKBitmap bitmap, Rectangle? rectangle = null)
        {
            if (bitmap == null)
            {
            ...
            //try
            //{
            PixelBytes = bitmap.BytesPerPixel;// Image.GetPixelFormatSize(bitmap.PixelFormat) / 8;
            Stride = Math.Abs(bitmap.RowBytes);
            BitData = new byte[bitmap.ByteCount];
            // Marshal.Copy(data.Scan0, BitData, 0, BitData.Length);
            Array.Copy(bitmap.Bytes, BitData, BitData.Length);
            //}
            //finally
            //{
        ...
        public virtual bool TryGetPixel(int x, int y, out Avalonia.Media.Color? color)
        {
            color = null;
            if (x < 0 || x >= Width || y < 0 || y >= Height)
            {
            ...
            var span = new byte[4];
            Buffer.BlockCopy(BitData, y * Stride + x * PixelBytes, span, 0, 4);
            color = Avalonia.Media.Color.FromArgb(span[3], span[2], span[1], span[0]);
            return true;
        }
    ...
    public class ClipBitmapData : BitmapData
    {
        ...
        public override bool TryGetPixel(int x, int y, out Avalonia.Media.Color? color)
        {
            return source.TryGetPixel(x + clip.X, y + clip.Y, out color);
        }
