    ...
    public static class StringHelper
    {
        ...
        public static decimal? GetDecimal(object obj)
        {
            switch (obj)
            {
                ...
                case long l:
                    return l;
                case string s when decimal.TryParse(s, out decimal si):
                    return si;
                default:
                    return null;
            }
        ...
        public static T[] GetEnumArray<T>(string text)
            where T : struct
        {
            return GetEnumArray<T>(text, ',');
        }

        public static T[] GetEnumArray<T>(string text, params char[] separators)
            where T : struct
        {
            if (string.IsNullOrEmpty(text))
            {
            ...
            var list = from s in strArray
                       where s != null
                       let str = s.Trim()
                       where !string.IsNullOrEmpty(str) && Enum.TryParse(str, out value)
                       select value;
        ...
        public static string? LimitStringLength(string str, int maxLength, bool singleLine = false, string overflowText = "…")
        {
            ...
            if (str.Length > maxLength)
            {
                if (maxLength > 0)
                {
                    str = str.Substring(0, maxLength - 1) + overflowText;
                }
                else
                {
                    str = string.Empty;
        ...
        public static bool IsValidFileName(string filename)
        {
            ...
            var illegal = Path.GetInvalidFileNameChars();
            return !filename.Any(c => illegal.Contains(c));
        }
